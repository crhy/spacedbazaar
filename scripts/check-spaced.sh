...
release = ET.parse(sys.argv[1]).getroot().find("releases/release")
assert release is not None and release.get("version") == sys.argv[2], "AppStream version differs from the binary"
PY
